...
const getSessions = (req, res) => {
  try {
    ...
    let userSessions = db.sesiones.filter((s) =>
...
const getSessionById = (req, res) => {
  try {
    ...
    if (!isUserAuthorizedForPlan(loggedInUserId, session.idPlan)) {
      return res
        .status(403)
        .json({ error: "No autorizado para acceder a esta sesión." });
    }
...
const createSession = (req, res) => {
  try {
    ...
    if (!idPlan || !fechaHora) {
      return res
        .status(400)
        .json({ error: "Faltan campos requeridos (idPlan, fechaHora)." });
    }

    if (!isUserAuthorizedForPlan(loggedInUserId, idPlan)) {
      return res
        .status(403)
        .json({ error: "No autorizado para crear una sesión para este plan." });
    }

    const newSession = {
      id:
        db.sesiones.length > 0
          ? Math.max(...db.sesiones.map((s) => s.id)) + 1
          : 1,
      idPlan,
      fechaHora,
  ...
  try {
    ...
    if (!idPlan || !fechaHora || !estado || !observaciones) {
      return res
        ...
        .json({
          error:
            "Faltan todos los campos requeridos para la actualización (PUT).",
        });
    }
    ...
    if (!isUserAuthorizedForPlan(loggedInUserId, originalSession.idPlan)) {
      return res
        .status(403)
        .json({ error: "No autorizado para modificar esta sesión." });
    }

    if (
      idPlan !== originalSession.idPlan &&
      !isUserAuthorizedForPlan(loggedInUserId, idPlan)
    ) {
      return res
        .status(403)
        .json({
          error: "No se puede mover la sesión a un plan que no te pertenece.",
        });
    }
...
const patchSession = (req, res) => {
  try {
    ...
    if (
      updates.idPlan &&
      updates.idPlan !== originalSession.idPlan &&
      !isUserAuthorizedForPlan(loggedInUserId, updates.idPlan)
    ) {
      return res
        .status(403)
        .json({
    ...
    res
      .status(500)
      .json({ error: "Error al actualizar parcialmente la sesión." });
  }
};
...
const deleteSession = (req, res) => {
  try {
    ...
    if (!isUserAuthorizedForPlan(loggedInUserId, sessionToDelete.idPlan)) {
      return res
        .status(403)
        .json({ error: "No autorizado para eliminar esta sesión." });
    }
